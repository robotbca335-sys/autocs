...
async function getStats() {
  ...
  const [total, pending, approved, rejected, todayCount] = await Promise.all([
    ...
    sb.from('claims').select('*', { count: 'exact', head: true }).eq('status', 'PENDING'),
    sb.from('claims').select('*', { count: 'exact', head: true }).eq('status', 'SESUAI'),
    sb.from('claims').select('*', { count: 'exact', head: true }).eq('status', 'TIDAK_SESUAI'),

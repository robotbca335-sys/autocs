 ...
 * Limitasi ini hanya berlaku per-instance â€” jika 10 instance aktif,
...
function wrapWithRateLimit(fn, bucket, send429) {
  const defaultSend = (res, result) => {
    ...
    res.end(JSON.stringify({ ok: false, message: 'Terlalu banyak permintaan â€” coba lagi nanti', retryAfter: result.retryAfter }));
  };
